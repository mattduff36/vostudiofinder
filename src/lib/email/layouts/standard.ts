/**
 * Standard Email Layout Renderer
 * 
 * This is the LOCKED layout used by most transactional emails.
 * Structure: Logo > Heading > Body > CTA(s) > Footer
 * 
...
function renderParagraphs(paragraphs: string[]): string {
  return paragraphs
    .map(p => {
      ...
      return `              <p style="margin: 0 0 24px 0; font-size: 16px; color: #4a4a4a; line-height: 1.6;">${withLineBreaks}</p>`;
    })
    .join('\n');
}
                ...
                <tr>
                  <td align="left" class="btn" style="border-radius: 6px;">
                    ...
                    <v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word"
                      href="${escapedUrl}" style="height:48px;v-text-anchor:middle;width:220px;" arcsize="12%"
...
/**
 * Render standard email layout
 */
export function renderStandardLayout(props: StandardLayoutProps): { html: string; text: string } {
  const {
    subject,
    preheader,
    heading,
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width: 600px; width: 100%; background-color: #ffffff; border-radius: 4px;">
          <tr>
            <td style="padding: 40px 40px 32px 40px;">
              ...
              <h1 style="margin: 0 0 16px 0; font-size: 24px; font-weight: 500; color: #1a1a1a; line-height: 1.3;">${escapedHeading}</h1>
