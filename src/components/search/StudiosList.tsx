...
interface Studio {
  ...
  studio_images: Array<{ image_url: string; alt_text?: string | null }>;
        ...
        {studios.map((studio) => {
              ...
              <div className="aspect-[25/12] bg-gray-200 rounded-t-lg overflow-hidden relative">
                ...
                <div className="absolute bottom-2 right-2 flex flex-wrap gap-1">
                  {studio.studio_studio_types.slice(0, 2).map((type, index) => (
                    ...
                    >
                      {type.studio_type.replace('_', ' ').toLowerCase().replace(/\b\w/g, (letter: string) => letter.toUpperCase())}
                    </span>
                  ))}
          ...
          if (profileHref) {
            return (
              <Link
                ...
                prefetch={false}
                className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-lg hover:border-primary-200 hover:scale-[1.02] transition-all duration-300 flex flex-col cursor-pointer group w-full"
          ...
          return (
            <div
              key={studio.id}
              id={`studio-${studio.id}`}
              className="bg-white border border-gray-200 rounded-lg shadow-sm transition-all duration-300 flex flex-col cursor-default group w-full"
              style={{ minHeight: '360px' }}
            >
              {cardContent}
            </div>
