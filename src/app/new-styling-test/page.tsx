...
export default function NewStylingTestPage() {
  const [isLoaded, setIsLoaded] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [heroOpacity, setHeroOpacity] = useState(0.85); // Default opacity (85%)
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: colors.background }}>
        ...
        <div className="relative text-white overflow-hidden">
          ...
          <div className="relative z-10 px-6 pt-32 pb-20">
            <div className="max-w-4xl mx-auto text-center">
              ...
              {/* Search Bar */}
              <form className={`max-w-4xl mx-auto mt-12 transition-all duration-1000 delay-600 ${
                isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
              }`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-16 max-w-3xl mx-auto">
                {[
                  { icon: Mic, title: 'Professional Studios', desc: 'Verified recording studios with professional equipment and acoustics' },
                  { icon: MapPin, title: 'Global Locations', desc: 'Find studios worldwide with precise location mapping and directions' },
                  { icon: Users, title: 'Direct Contact', desc: 'Connect directly with studio owners and book sessions instantly' }
                ].map((feature, index) => (
                  <div key={index} className={`text-center transition-all duration-1000 ${
                    ...
                    <p style={{ color: '#f5f5f5' }}>{feature.desc}</p>
                  </div>
                ))}
              </div>
            </div>
                ...
                <div key={studio.id} className="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-xl hover:border-gray-300 hover:scale-[1.02] transition-all duration-300 flex flex-col h-full cursor-pointer group">
                  ...
                  <div className="aspect-video bg-gray-200 rounded-t-xl overflow-hidden relative">
                    {studio.images?.[0]?.imageUrl ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center" style={{ color: colors.textSubtle }}>
                        <Users className="w-12 h-12" />
                      </div>
                    )}
                    {/* Studio Type Badge - moved to image */}
                    <div className="absolute bottom-3 right-3">
                      <span 
                        className="inline-block px-3 py-1 text-white text-xs font-medium rounded-full shadow-lg"
                  ...
                  <div className="p-6 flex flex-col flex-grow max-h-[500px]">
                    ...
                    <div className="mb-4">
                      <div className="flex flex-wrap gap-1">
                        {studio.services.slice(0, 2).map((service, index) => (
                          <span key={index} className="inline-block px-2 py-1 bg-gray-100 text-xs rounded" style={{ color: colors.textSecondary }}>
                    ...
                    <div className="flex items-center justify-between mt-auto pt-2">
                      ...
                      <div 
                        className="px-3 py-1.5 text-sm font-medium rounded-lg group-hover:shadow-md transition-all duration-300 pointer-events-none"
                        style={{ backgroundColor: colors.primary, color: colors.background }}
                      >
                        View Details
                      </div>
        ...
        <div className="relative py-16 overflow-hidden">
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-6">
            <div className="text-center text-white mb-12">
              ...
              <p className={`text-xl max-w-3xl mx-auto transition-all duration-1000 ease-out ${
                isLoaded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-20'
              }`} style={{ transitionDelay: '0.2s', color: '#f5f5f5' }}>
              ...
              {[
                {
                  ...
                  items: ['Find studios near you or worldwide', 'Compare equipment and services', 'Read reviews from other artists', 'Book sessions directly with studios']
                },
                {
                  icon: Building,
                  title: 'For Studio Owners',
                  items: ['Reach thousands of voice artists worldwide', 'Professional listing with photos & details', 'Direct bookings from qualified clients', 'Only £25/year - exceptional value']
                },
                {
                  icon: Users,
                  title: 'For Everyone',
                  items: ['Join a global community', 'Network with professionals', 'Share experiences and tips', 'Stay updated on industry trends']
                }
              ].map((section, index) => (
                <div key={index} className="text-center text-white">
                  ...
                  <ul className="space-y-2 text-sm" style={{ color: '#f5f5f5' }}>
      ...
      {/* Footer */}
      <footer style={{ backgroundColor: '#1a1a1a', color: colors.background }}>
        <div className="max-w-7xl mx-auto px-6 py-12">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div className="col-span-1 md:col-span-2">
              ...
              <p className="mb-6 max-w-md" style={{ color: '#cccccc' }}>
                The world's leading platform for connecting voice artists with professional recording studios. 
                Find your perfect studio or showcase your space to a global community of voice professionals.
              </p>
            </div>
            <div>
