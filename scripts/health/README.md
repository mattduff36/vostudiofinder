...
The Next.js floor is a numeric major/minor/patch comparison, not a string compare. Update `NEXT_MIN_SECURE` in `health.mjs` when a later required secure release is adopted. Shared semver helpers live in `scripts/health/semver-floor.cjs`: any `X.Y.Z-<prerelease>` suffix is a prerelease, including unnamed forms such as `4.24.15-dev.1`. The Proxy convention check PASSes when only `src/proxy.ts` exists and FAILs if `src/middleware.ts` returns, both files exist, or neither convention is present.
